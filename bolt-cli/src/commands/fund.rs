...
impl FundCommand {
    ...
    pub async fn run(self) -> Result<()> {
        ...
        let amount = parse_ether(&self.amount.to_string())
            .wrap_err("failed to parse ETH amount")?;
        ...
        for (i, private_key_hex) in TEST_ACCOUNTS.iter().enumerate() {
            let target_wallet: PrivateKeySigner = 
                format!("0x{}", private_key_hex).parse()
                    .wrap_err_with(|| format!("invalid private key for account {}", i + 1))?;
            
            let target_address = target_wallet.address();
            
            info!("Account {}: {} (funding with {} ETH)", 
                  i + 1, target_address, self.amount);
