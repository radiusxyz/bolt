...
mod tests {
    use crate::{
        ...
        primitives::{
            commitment::SignedCommitment, ExclusionRequest, FirstInclusionRequest,
        },
    };
    use alloy::{
        primitives::Address,
    ...
    #[test]
    fn test_exclusion_request_with_none_access_list() {
        // Test that exclusion requests with None access_list are handled correctly
        let exclusion_request = ExclusionRequest {
            slot: 12345,
            txs: vec![], // Empty for simplicity
            access_list: None, // This should trigger access list fetching
            signature: None,
            signer: None,
        };

        // Verify the access list is None initially
        assert!(exclusion_request.access_list.is_none());
        
        // Test the logic that would be used in the driver
        let should_fetch_access_list = exclusion_request.access_list.is_none() && !exclusion_request.txs.is_empty();
        assert!(!should_fetch_access_list); // False because txs is empty
    }

    #[test]
    fn test_first_inclusion_request_with_none_access_list() {
        // Test that first inclusion requests with None access_list are handled correctly
        let first_inclusion_request = FirstInclusionRequest {
            slot: 12345,
            txs: vec![], // Empty for simplicity
            access_list: None, // This should trigger access list fetching
            bid_transaction: vec![], // Empty for simplicity
            signature: None,
            signer: None,
        };

        // Verify the access list is None initially
        assert!(first_inclusion_request.access_list.is_none());
        
        // Test the logic that would be used in the driver
        let should_fetch_access_list = first_inclusion_request.access_list.is_none() && !first_inclusion_request.txs.is_empty();
        assert!(!should_fetch_access_list); // False because txs is empty
        
        // Test transaction combination logic
        let mut all_txs = first_inclusion_request.txs.clone();
        all_txs.extend(first_inclusion_request.bid_transaction.clone());
        assert_eq!(all_txs.len(), 0); // Both sets are empty
    }
